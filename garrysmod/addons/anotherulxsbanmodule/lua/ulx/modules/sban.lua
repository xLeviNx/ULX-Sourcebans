...
local ipCache = {}
local prefix = database_sban:escape(SBAN_PREFIX)
...
local function ReportBlock(bid, name)
	...
	qTab.wait = false
	local q = [[
		INSERT INTO %sbanlog (sid, time, name, bid)
		VALUES (
			'%s', %d, '%s', %d
		);
	]]
	
	SBAN_SQL_Query(string.format(q, prefix, database_sban:escape(SBAN_SERVERID), database_sban:escape(ostime), 
		database_sban:escape(name), database_sban:escape(bid)), qTab)
end
...
function SBAN_getadmin_id(steamid)
	...
	if found and found.sb_aid then return found.sb_aid end
	
	local qTab = {}
	qTab.wait = true
	local q = [[
		SELECT aid FROM %sadmins
			WHERE authid = '%s'
	]]
	local query = SBAN_SQL_Query(string.format(q, prefix, database_sban:escape(steamid)), qTab)
...
function SBAN_canunban(steamid, ply)
	...
	q = string.format(q, prefix, database_sban:escape(steamid))
    if not ULib.ucl.query(ply, "ulx unsbanall") then
		q = q .. string.format(" AND aid = %d", database_sban:escape(adminID))
    end

    local qTab = {}
    qTab.wait = true
    
    return SBAN_SQL_Query(q, qTab) ~= nil
end
...
function SBAN_updateban(steamID, ply, bantime, reason, name)
	...
	SBAN_SQL_Query(string.format(q, prefix, bantime, bantime, updateName, reason, steamID), qTab)
	XGUIRefreshBans()
end
-- ############### Unban UI  Section ###############
-- #################################################
...
function SBAN_RetrieveBans()
	...
	local q = [[
		...
		b.aid, b.bid, b.sid, b.name, 
		b.reason, b.authid, b.created, b.ends
		FROM %sbans b
			INNER JOIN %sadmins a
			ON b.aid = a.aid
			WHERE b.RemoveType IS NULL
				ORDER BY b.created
		DESC LIMIT %d
	]]

	SBAN_SQL_Query(string.format(q, prefix, prefix, banRetrieveLimit), qTab)
end
...
local function CheckAdmin(result, qTab, lev)
	...
	if not lev then -- Initial call
		if #result == 1 then
			...
			local q = [[
				SELECT admin_id FROM %sadmins_servers_groups
					WHERE admin_id = %d
						AND srv_group_id = -1
						AND server_id = %d
			]]
			SBAN_SQL_Query(string.format(q, prefix, 
				database_sban:escape(result[1].aid), database_sban:escape(SBAN_SERVERID)), qTab)
		else
	...
	elseif lev == "checkadmingroup" then
		if #result >= 1 then --Admin has permissions for this specific server, finding admin group to add.
			...
			local q = [[
				SELECT srv_group FROM %sadmins
					WHERE authid = '%s'
				]]
			SBAN_SQL_Query(string.format(q, prefix, steamid), qTab)
			return
		else
			...
			SELECT * FROM %sservers_groups sg 
				...
				AND (
					SELECT asg.srv_group_id FROM %sadmins_servers_groups asg 
						WHERE admin_id = '%s' 
							AND asg.srv_group_id = sg.group_id
				) >= 1;
			]]
			local servermatchq = string.format(qS, prefix, 
				database_sban:escape(SBAN_SERVERID), prefix, database_sban:escape(aid))
	...
	elseif lev == "servergroupmatch" then
		if #result >= 1 then
			qTab.cb = function(result, qTab) 
				CheckAdmin(result, qTab, "addtogroup")
			end
...
local function StartAdminCheck(ply, steamid)
	...
	SBAN_SQL_Query(string.format(q, prefix, database_sban:escape(steamid)), qTab)
end
...
local function StartBanCheck(ply, steamID)
	...
	local q = [[
		SELECT 
			bid, authid, ends, length, reason, RemoveType
		FROM %sbans
		WHERE
			authid = '%s'
	]]
	...
	if checkIP and not ipCache[qTab.ip] then
		...
		SBAN_SQL_Query(string.format(q, prefix, database_sban:escape(qTab.ip)), qTab)
